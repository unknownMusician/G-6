...
namespace G6.Characters {
    public abstract class CharacterBase : MonoBehaviour
    {
        ...
        public List<InteractableBase> GetPosibleInteractions() {
            return FindObjectsOfType<InteractableBase>()
                    .Where((interactable) => (this.transform.position - interactable.transform.position).magnitude < InteractionRadius)
                    .OrderBy(p => (this.transform.position - p.transform.position).magnitude)
                    .ToList();
        }
